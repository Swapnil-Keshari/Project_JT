...
Dict1=ph.to_dict('series')

Dict2={}
Dict2={gene:{} for gene in set(ph['Gene'])}
...
for gene in Dict2.keys():
    ...
    for seq in Dict2[gene].keys():
        ...
        for phospho in Dict2[gene][seq]:
            for index in range(len(Dict1["Gene"])):
                if (str(seq)+'_'+str(phospho)) == Dict1["Sequence1"][index]:
                    Dict2[gene][seq][phospho].append(ph.iloc[[index]].select_dtypes(exclude=[np.object]))
    for seq in Dict2[gene].keys():
        for ele in Dict2[gene][seq].keys():
            ...
            if (re.match('1xS\d|1xT\d|1xS/T', ele) is None) and '1x' in ele:
                for ele1 in Dict2[gene][seq].keys():
                    if (re.match('1xS\d|1xT\d',str(ele1)) is not None) and '1x' in str(ele1):
                        ...
                        break
            elif 'S/T' in ele and '2x' in ele and '2xS/T' not in ele:
                s = ele.replace('2x','')
